...
class OsobaSerializer(serializers.ModelSerializer):
    def validate_imie(self, value):
        if not value.isalpha():
            raise serializers.ValidationError("Pole 'imie' Bałwanie domyśl się, nie może imie być jak dziecko Elona Muska!!!")
        return value
    
    def validate_nazwisko(self, value):
        if not value.isalpha():
            raise serializers.ValidationError("Pole 'nazwisko' Naprawdę? Tylko litery!!!")
        return value
    
    def validate_data_dodania(self, value):
        if value > date.today():
            raise serializers.ValidationError("Pole 'data_dodania' Ty debilu, nie możesz dodać osoby z przyszłości!!!")
        return value
    class Meta:
        model = Osoba
